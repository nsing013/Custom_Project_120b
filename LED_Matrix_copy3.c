...
unsigned char row_val = 0x01;															//this is the row where the cursor is
unsigned char temp_val1, temp_val2, temp_val3;
int i = 0;

enum LED_Matrix_States {LED_Matrix_Start, LED_Matrix_cursor, LED_Matrix_move_right, LED_Matrix_move_left, LED_Matrix_move_up, LED_Matrix_move_down, LED_Matrix_levels, LED_Matrix_update_user_matrix};
int LED_Matrix_Tick(int state)
{
	...
	switch (state) //Actions
	{
		...
		case LED_Matrix_cursor:
			...
			//also want to show the cursor with the user input
			
			/*
			if(i == row_val)
			{
				PORTC = user_input [row_val] | cursor;
			}
			else
			{
				PORTC = user_input [row_val]
			*/
		...
		case LED_Matrix_levels:				//dont screw with this
			...
			break;
		case LED_Matrix_update_user_matrix:
			temp_val1 = ~(user_input[row_val]);		//invert original number in the  //should i use i?
			temp_val2 = ~(cursor);					//invert current value when button is pressed
			temp_val3 = temp_val1 | temp_val2;		//or both numbers
			user_input [i] = ~(temp_val3);			//update original number
			//PORTD = 0x01 << i;					//send power to all the rows
			break;
		default:
			break;
	}
